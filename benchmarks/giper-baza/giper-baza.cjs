...
(function ($_1) {
     ...
     */
    class $giper_baza_crdtbench extends $mol_object2 {
        ...
        static auth_next() {
            ...
            if (!str)
                return $mol_fail(new Error('Auth pool is empty'));
     ...
     */
    class $giper_baza_crdtbench_land extends $giper_baza_land {
        ...
        units_signing() {
            ...
            for (let guard = 0;; ++guard) {
                if (guard > 1024)
                    return $mol_fail(new Error('Endless signing'));
        ...
        units_seal(units) {
            ...
            for (const chunk of $mol_array_chunks(units.map(unit => unit.hash()), $giper_baza_unit_seal_limit)) {
                ...
                do {
                    ...
                    seal.sign($_1.$giper_baza_crdtbench_sign(auth.signer(), sens));
                } while (seal.rate_min() > rate);
                for (const hash of seal.hash_list())
                    seal.alive_items.add(hash.str);
        ...
        units_flush() {
            ...
            if (face)
                faces.set(pass.peer().str, face.clone());
            return $giper_baza_pack.make([[
                    this.link().str,
        ...
        sands_open(sands) {
            for (const sand of sands) {
                if (sand._open)
                    continue;
                if (!sand._ball)
                    sand._ball = sand.big() ? this.mine().ball_load(sand) : sand.data();
                sand._open = sand._ball;
            }
            return undefined;
        }
    }
    $_1.$giper_baza_crdtbench_land = $giper_baza_crdtbench_land;
    /** One benchmarked client: own `$` context, own Auth, shared Land. */
    class $giper_baza_crdtbench_doc {
        update;
        $;
        land;
        text;
        ...
        dict_read() {
            ...
            for (const key of this.dict.keys()) {
                ...
                if (atom)
                    res[String(key)] = atom.vary();
            }
            return res;
        }
    }
    $_1.$giper_baza_crdtbench_doc = $giper_baza_crdtbench_doc;
    /**
     ...
     */
    $_1.$giper_baza_crdtbench_sign = (key, data) => $mol_fail(new Error('Synchronous signer is not installed'));
    /** Synchronous Ed25519 verification. */
    $_1.$giper_baza_crdtbench_verify = (key, data, sign) => $mol_fail(new Error('Synchronous auditor is not installed'));
    function $giper_baza_crdtbench_crypto(sign, verify) {
        $_1.$giper_baza_crdtbench_sign = sign;
        $_1.$giper_baza_crdtbench_verify = verify;
        const signer = $mol_crypto2_signer.prototype;
        signer.sign = function (data) {
            return $_1.$giper_baza_crdtbench_sign(this, data);
        };
        const auditor = $mol_crypto2_auditor.prototype;
        auditor.verify = function (data, sign) {
            return $_1.$giper_baza_crdtbench_verify(this, data, sign);
        };
    }
    $_1.$giper_baza_crdtbench_crypto = $giper_baza_crdtbench_crypto;
})($ || ($ = {}));
